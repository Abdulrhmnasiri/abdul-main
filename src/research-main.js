...
import { applyResearchTranslations } from './modules/research-i18n.js';
...
function initMobileFileStrip() {
  const toggle = document.getElementById('mobileFilesToggle');
  const strip = document.getElementById('mobileFilesStrip');
...
function initActiveFileTracking() {
  const sections = Array.from(document.querySelectorAll('.dossier-main .dossier-section[id^="sec-"]'));
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    setCurrentYear();
    applyResearchTranslations(currentLang);
    ...
    console.error('Research page init failed:', err);
